...
import {useVideoArrays} from './POC_VideoArrayRenderer';
...
const GridVideo = ({setLayout}: GridVideoProps) => {
  ...
  let {matrix, dims} = useMemo(
    () => layout(totalUsers, isDesktop),
    [totalUsers, isDesktop],
  );

  const videoArray = getVideoArray({setLayout, dims, layout: Layout.Grid});
  return (
    <View
      ...
      {matrix.map((r, ridx) => (
        <View style={style.gridRow} key={ridx}>
          {r.map((_, cidx) => videoArray[ridx * dims.c + cidx])}
        </View>
      ))}
    </View>
  );
